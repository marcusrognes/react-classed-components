...
function processTemplateLiteral<T>(
	strings: TemplateStringsArray,
	calls: (string | ((props: any) => string))[],
	context: T,
) {
	let classNamesList = [];
...
function elementConstructor(
	...
	strings: TemplateStringsArray,
	...calls: (string | ((props: T) => string))[]
) => ({
	className,
	...props
}: ComponentProps<SupportedHTMLElements> & T) => ReactElement<
	{},
...
> {
	...
	return (
		...
		className,
		...props
	}: ComponentProps<SupportedHTMLElements>) => ReactElement<
		{},
		string | JSXElementConstructor<any>
	>) => {
		return ({ className, ...props }) => {
			...
			return createElement(tag, {
				...(props as any),
				className: classNameList.map((c) => c.trim()).join(" "),
			});
		};
	};
}
...
type Test<
	...
	U = ReturnType<typeof elementConstructor>,
> = { [P in T[number & keyof T] & PropertyKey]: U };
